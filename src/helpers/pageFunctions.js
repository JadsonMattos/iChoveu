...
export function createCityElement(cityInfo) {
  ...
  async function getForecastCity(cityURL) {
    const TOKEN = import.meta.env.VITE_TOKEN;
    const urlAPI = `https://api.weatherapi.com/v1/forecast.json?lang=pt&key=${TOKEN}&q=${cityURL}&days=7`;
  ...
  // btnForecast.textContent = 'Ver previsão';
  btnForecast.addEventListener('click', async () => {
    const forecastList = await getForecastCity(url);
